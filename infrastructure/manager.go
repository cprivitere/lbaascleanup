...
import (
	...
	lbaas "github.com/cprivitere/lbaascleanup/v1"
)
...
func (m *Manager) DeleteLoadBalancer(ctx context.Context, id string) (map[string]string, error) {
	outputProperties := map[string]string{}
	ctx = context.WithValue(ctx, lbaas.ContextOAuth2, m.tokenExchanger)
...
func (m *Manager) DeleteLoadBalancerPool(ctx context.Context, id string) (map[string]string, error) {
	outputProperties := map[string]string{}
	ctx = context.WithValue(ctx, lbaas.ContextOAuth2, m.tokenExchanger)
	...
	// TODO lb, resp, err :=
	_, err := m.client.PoolsApi.DeleteLoadBalancerPool(ctx, id).Execute()
	if err != nil {
		return nil, err
	}
